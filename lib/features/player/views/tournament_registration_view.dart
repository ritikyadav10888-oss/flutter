...
class _TournamentRegistrationViewState
  ...
  Widget _buildInfoCard(AppUser user) {
    // Only show player-specific info if the user has the player role
    final hasPlayerRole = user.roles.contains(UserRole.player);
      ...
      child: Column(
        children: [
          ...
          _buildDetailRow(
            ...
            'Gender',
            hasPlayerRole ? (user.gender ?? 'N/A') : 'N/A',
          ),
          const Divider(height: 24, color: Colors.black12),
          _buildDetailRow(
            Icons.cake,
            'Age',
            hasPlayerRole ? (user.age?.toString() ?? 'N/A') : 'N/A',
          ),
          const Divider(height: 24, color: Colors.black12),
          _buildDetailRow(
            Icons.military_tech_rounded,
            'Position',
            hasPlayerRole ? (user.playingPosition ?? 'N/A') : 'N/A',
          ),
          const Divider(height: 24, color: Colors.black12),
          _buildDetailRow(
            Icons.credit_card,
  ...
  Future<void> _register() async {
    ...
    try {
      final registration = Registration(
        ...
        ownerId: widget.tournament.createdBy,
      );

      await TournamentService().registerPlayer(registration);
